...
For separate frontend and API hosts, set `VITE_API_URL` to the API origin *before building* the frontend and set `FRONTEND_ORIGIN` on the API to the exact frontend origin (comma-separated if there are several). Credentialed CORS never uses `*`. If the hosts are on different sites, set `SESSION_COOKIE_SAME_SITE=none` and use HTTPS so the session cookie can be sent. Configure the frontend hostname in Firebase Authentication's authorized domains. There is no active backend OAuth callback endpoint.
